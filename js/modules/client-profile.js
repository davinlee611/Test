...
function handleProfileSubmit(event) {
  ...
  if (!validationResult.isValid) {
    showProfileMessage(validationResult.message);
...
function validateProfile() {
  ...
  const selectedDate = createLocalDate(profile.dateOfBirth);
  ...
  if (!profile.gender) {
    return {
      isValid: false,
      message: "Please select the client's gender.",
      element: genderInput,
    };
  }
  ...
  if (!profile.maritalStatus) {
    return {
      isValid: false,
      message: "Please select the client's marital status.",
  ...
  if (!profile.occupation) {
    return {
      isValid: false,
      message: "Please enter the client's occupation.",
  ...
  if (!profile.employmentStatus) {
    return {
      isValid: false,
      message: "Please select the client's employment status.",
...
export function renderProfile() {
  ...
  setInputValue(dateOfBirthInput, profile.dateOfBirth);

  setInputValue(genderInput, profile.gender);

  setInputValue(maritalStatusInput, profile.maritalStatus);

  setInputValue(occupationInput, profile.occupation);

  setInputValue(employmentStatusInput, profile.employmentStatus);

  setInputValue(dependantsInput, profile.dependants || "");
...
export function showProfileMessage(message) {
